...
public class Esutil {
	public static Client client = null;
		...
		public static  Client getClient() {
			...
			Settings settings = Settings.settingsBuilder().put("cluster.name", "myclu").build();
			try {
				client = TransportClient.builder().settings(settings).build()
						//这块只能用hostname，不能用IP
						.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("node1"), 9300))
						.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("node6"), 9300))
						.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName("node7"), 9300));
